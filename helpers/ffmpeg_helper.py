...
async def MergeVideo(input_file: str, user_id: int, message: Message, format_: str):
    ...
    """
    output_vid = f"downloads/{str(user_id)}/[@Madflix_Bots].{format_.lower()}"
...
async def MergeSub(filePath: str, subPath: str, user_id):
    ...
    subTrack += 1
    subTitle = f"Track {subTrack} - tg@Madflix_Bots"
    ...
    orgFilePath = shutil.move(
        f"downloads/{str(user_id)}/[@Madflix_Bots]_softmuxed_video.mkv", filePath
    )
    return orgFilePath
...
def MergeSubNew(filePath: str, subPath: str, user_id, file_list):
    ...
    for j in range(1, (len(file_list))):
        ...
        muxcmd.append(f"{j}:s")
        muxcmd.append(f"-metadata:s:s:{subTrack}")
        muxcmd.append(f"title=Track {subTrack+1} - tg@Madflix_Bots")
        subTrack += 1
    muxcmd.append("-c:v")
    muxcmd.append("copy")
    muxcmd.append("-c:a")
    ...
    subprocess.call(muxcmd)
    return f"downloads/{str(user_id)}/[@Madflix_Bots]_softmuxed_video.mkv"
...
def MergeAudio(videoPath: str, files_list: list, user_id):
    ...
    for j in range(1, len(files_list)):
        ...
        muxcmd.append(f"title=Track {audioTracks+1} - tg@Madflix_Bots")
        audioTracks += 1
    ...
    LOGGER.info(process)
    return f"downloads/{str(user_id)}/[@Madflix_Bots]_export.mkv"
